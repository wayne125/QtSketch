...
for x in queries_file:
    try:
        ...
        print(
            "Cannot load query molecules from SMARTS\n  %s\n  Error: %s"
            % (x, getIndigoExceptionText(e))
        )
    cnt += 1
...
for t, ts in zip(targets, targets_unser):
    ...
    for q in queries:
        ...
        if (cnt > 0) != (m2 != None):
            print(
                "  %s %s: Matching is different for the original and unserialized molecule, %s %s"
        ...
        if cnt != cnt2:
            print(
                "  %s %s: different match count: %d and %d for optimized"
                % (t.name(), q.name(), cnt, cnt2)
            )
