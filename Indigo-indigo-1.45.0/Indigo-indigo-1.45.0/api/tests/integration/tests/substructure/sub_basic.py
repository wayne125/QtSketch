...
def testSSS(mol, q):
    ...
    try:
        ...
        if (match != None) != (match_opt != None):
            msg = (
                "match before and after optimization is different: match=%s and match_opt=%s"
                % (match, match_opt)
            )
            print(msg)
        ...
        if cnt != cnt_opt:
            msg = (
                "count before and after optimization is different: cnt=%d and cnt_opt=%d"
                % (cnt, cnt_opt)
            )
            print(msg)
            sys.stderr.write(msg + "\n")
        ...
        if (cnt > 0) != match:
            msg = (
                "match and countMatches returns contradicting results: match=%s and count=%d"
                % (match, cnt)
            )
            print(msg)
            sys.stderr.write(msg + "\n")
...
tests = [
    ...
    (
        lmol(
            "c1cc2cc3ccc4cc5cc6cccc7cc8ccc9cc%10cc(c1)c2c1c3c4c2c5c(c67)c8c9c2c%101"
        ),
        lqmol("*~*~*~*~*~*~*~*~*~*~*~*~*~*~*"),
    ),
    (
    ...
    (
        lmol("c1cc2concc2cn1"),
        lqmolf(joinPathPy("molecules/r1_2ap.mol", __file__)),
    ),
    (
        lmol("c1cc2cnocc2cn1"),
        lqmolf(joinPathPy("molecules/r1_2ap_aal.mol", __file__)),
    ),
    (
        lmolf(joinPathPy("molecules/r2_target.mol", __file__)),
        lqmolf(joinPathPy("molecules/r2.mol", __file__)),
    ),
    (lmol("c1ccccc1"), lsmarts("[#6]cccc[#6,#7]")),
    (
        lmol("c1ccccc1"),
        lqmolf(joinPathPy("molecules/q_rg_recurs.mol", __file__)),
    ),
    (
        lmol("c1ccccc1.c1ccccc1"),
        lqmolf(joinPathPy("molecules/q_rg_recurs.mol", __file__)),
    ),
    (
        lmol("c1ccccc1"),
        lqmolf(joinPathPy("molecules/q_rg_recurs2.mol", __file__)),
    ),
    (
        lmol("C1CCCCCC1"),
        lqmolf(joinPathPy("molecules/q_rg_recurs2.mol", __file__)),
    ...
    (
        lmol("C1CCCCCC1.C1CCCCCC1"),
        lqmolf(joinPathPy("molecules/q_rg_recurs2.mol", __file__)),
    ),
    (
        lmol("OC(=O)C1=CC=CC=C1"),
        lqmolf(joinPathPy("molecules/rgroups/c11100_3.mol", __file__)),
    ),
    ...
    (
        lmolf(joinPathPy("molecules/test_basic_smarts.mol", __file__)),
        lsmarts("[Fe]~N"),
    ),
    (
        lmolf(joinPathPy("molecules/test_basic_smarts.mol", __file__)),
    ...
    (
        lmolf(joinPathPy("molecules/bad_valance.mol", __file__)),
        lsmarts("[#7&v5]=O"),
    ),
    (
        lmol("C12SC3N(C=CN=3)C=1C(C1=C(C=CC(O)=C1C2=O)Cl)=O"),
        lqmol("*1*******1"),
    ),
    (
        lmol("C12SC3N(C=CN=3)C=1C(C1=C(C=CC(O)=C1C2=O)Cl)=O"),
...
def testSSSWithSerializedAndAromatized(mol, q):
    ...
    if len(set(counts)) != 1:
        msg = "number of matches is different after serialization: " + str(
            counts
        )
        print(msg)
        sys.stderr.write(msg + "\n")
