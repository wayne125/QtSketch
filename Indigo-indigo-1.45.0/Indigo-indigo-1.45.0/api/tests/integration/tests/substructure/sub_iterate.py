...
def checkMatch(query, mapping):
    ...
    for query_atom in query.iterateAtoms():
        ...
        if query_atom.atomicNumber() != target_atom.atomicNumber():
            print(
                "Error! {0} -> {1}; atomic numbers differ: {2} {3}".format(
                    query_atom.index(),
                    mapped_ind,
    ...
    for query_bond in query.iterateBonds():
        ...
        if (
            d[query_bond.source().index()] == target_bond.source().index()
            and d[query_bond.destination().index()]
            == target_bond.destination().index()
        ) or (
            d[query_bond.source().index()] == target_bond.destination().index()
            and d[query_bond.destination().index()]
            == target_bond.source().index()
        ):
            if (
                query_bond.bondOrder() == target_bond.bondOrder()
                or query_bond.bondOrder() == 4
                or target_bond.bondOrder() == 4
            ):
                order = ["-", "=", "#", ":"][query_bond.bondOrder() - 1]
                print(
                    "Bond {1}{0}{2} --> {3}{0}{4}".format(
                        order,
                        query_bond.source().index(),
