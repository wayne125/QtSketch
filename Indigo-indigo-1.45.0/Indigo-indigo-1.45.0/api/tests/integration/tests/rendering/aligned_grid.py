...
refatoms = []
for structure in indigo.iterateSDFile(
    joinPathPy("molecules/benzodiazepine.sdf.gz", __file__)
):
...
renderer.renderGridToFile(
    collection, None, 4, joinPathPy("out/grid.svg", __file__)
)
print(checkImageSimilarity("grid.svg"))
indigo.setOption("render-output-format", "png")
renderer.renderGridToFile(
    collection, None, 4, joinPathPy("out/grid.png", __file__)
)
...
renderer.renderGridToFile(
    collection, refatoms, 4, joinPathPy("out/grid1.svg", __file__)
)
print(checkImageSimilarity("grid1.svg"))
indigo.setOption("render-output-format", "png")
renderer.renderGridToFile(
    collection, refatoms, 4, joinPathPy("out/grid1.png", __file__)
)
...
for alignment in options_align:
    ...
    renderer.renderGridToFile(
        collection, None, 4, joinPathPy("out/" + fname, __file__)
    )
