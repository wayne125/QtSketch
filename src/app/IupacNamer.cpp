...
enum class GroupType {
    ...
    PHOSPHINIC_ACID, // Phosphinic acid
        ...
        for (int i = 0; i < ringSize; ++i) {
            ...
            for (size_t j = 0; j < g.nodes[rNode].neighbors.size(); ++j) {
                ...
                if (nZ == 8 && order == 2) {
                    ...
                    int dblO = 0, dblS = 0, dblN = 0, sglO_OH = 0, sglN_NH2 = 0;
                    ...
                    } else if (g.nodes[nei].neighbors.size() == 3 && dblN == 1 && sglN_NH2 == 1) {
                    ...
                    if (g.nodes[nei].neighbors.size() == 4 && dblO == 2 && sglO_OH == 1) {
                        subName = "sulfo";
                    } else if (g.nodes[nei].neighbors.size() == 3 && dblO == 1 && sglO_OH == 1) {
                        subName = "sulfino";
                    } else {
                        subName = nameBranchGraph(g, nei, rNode, allIndependentRings, combinedForbidden);
                    }
                                    ...
                                    int acylHalideHalogenZ)
{
    ...
    if (winningType == GroupType::SULFONIC_ACID) {
    ...
    } else if (winningType == GroupType::SULFONAMIDE) {
        if (pCount == 1) sfx = (k <= 2) ? QStringLiteral("sulfonamide")
                                        : QString("-%1-sulfonamide").arg(principalLocants[0]);
        else {
            QStringList lStrs;
            for (int l : principalLocants) lStrs.append(QString::number(l));
            sfx = QString("-%1-%2sulfonamide").arg(lStrs.join(","), multiPrefix(pCount));
        }
        ...
        if (pCount == 1) sfx = (k <= 2) ? QString("arsonic %1").arg(hName)
                                        : QString("-%1-arsonic %2").arg(principalLocants[0]).arg(hName);
        else {
            QStringList lStrs;
            for (int l : principalLocants) lStrs.append(QString::number(l));
            sfx = QString("-%1-%2arsonic %3").arg(lStrs.join(","), multiPrefix(pCount), hName);
        }
    ...
    } else if (winningType == GroupType::THIOAMIDE) {
...
QString nameChainParentWithRingSubstituent(int mol, const std::map<int, int> &indigoToGraphIdx,
    ...
    } else if (winningType == GroupType::SULFONYL_HALIDE || winningType == GroupType::SULFINYL_HALIDE) {
        for (int pc : principalCarbons) {
            for (size_t i = 0; i < g.nodes[pc].neighbors.size(); ++i) {
                int nei = g.nodes[pc].neighbors[i];
...
IupacResult IupacNamer::generateName(int mol) {
        ...
        if (countP == 2 && countO == 7) {
            ...
            for (size_t i = 0; i < g.nodes.size(); ++i) {
                if (g.nodes[i].atomicNumber == 8 && g.nodes[i].neighbors.size() == 2) {
                    ...
                    if (g.nodes[nei1].atomicNumber == 15 && g.nodes[nei2].atomicNumber == 15 && g.nodes[i].totalH == 0) {
    ...
    // --- N-substituted exocyclic amines on saturated N-heterocycle ring parent ---
    {
        int countN = 0, countOtherHeavy = 0, countHalogen = 0;
        for (const auto &n : g.nodes) {
            if (n.atomicNumber == 7) countN++;
            else if (n.atomicNumber == 9 || n.atomicNumber == 17 || n.atomicNumber == 35 || n.atomicNumber == 53) countHalogen++;
        ...
        if (countN == 2 && countOtherHeavy == 0 && countHalogen == 0) {
            ...
            if (nNode1 != -1 && nNode2 != -1) {
                ...
                int ringN = -1;
                int exoN = -1;
                std::set<int> parentRing;
                for (const auto& r : sssrRings) {
                    bool n1InRing = r.count(nNode1);
                    bool n2InRing = r.count(nNode2);
                    if (n1InRing && !n2InRing) {
                        ringN = nNode1; exoN = nNode2; parentRing = r; break;
                    } else if (!n1InRing && n2InRing) {
                        ringN = nNode2; exoN = nNode1; parentRing = r; break;
                    }
                }
                
                if (ringN != -1 && exoN != -1) {
                    bool bondedToSameRing = false;
                    for (int nei : g.nodes[exoN].neighbors) {
                        if (parentRing.count(nei)) {
                            bondedToSameRing = true;
                            break;
                        }
                    }
                    ...
                    if (bondedToSameRing) {
                        std::vector<int> alkylNeighbors;
                        for (size_t j = 0; j < g.nodes[exoN].neighbors.size(); ++j) {
                            int nei = g.nodes[exoN].neighbors[j];
                            if (!parentRing.count(nei) && g.nodes[nei].atomicNumber == 6 && g.nodes[exoN].bondOrders[j] == 1) {
                                alkylNeighbors.push_back(nei);
                            }
                        }
                        
                        if (alkylNeighbors.size() > 0 && alkylNeighbors.size() <= 2) {
                            struct ExoBranch {
                                int alkylNeighbor;
                                int len;
                                int ringNode;
                            ...
                            for (int cNode : alkylNeighbors) {
                                ...
                                if (isRingNode) {
                                    ...
                                    while (true) {
                                        ...
                                        for (size_t j = 0; j < g.nodes[curr].neighbors.size(); ++j) {
                                            int nei = g.nodes[curr].neighbors[j];
                                            if (nei == prev) continue;
                                            if (g.nodes[curr].bondOrders[j] != 1) { invalidBranch = true; break; }
                                            if (g.nodes[nei].atomicNumber > 1) {
                                                heavyCount++;
                                                bool isR = false;
                                                for (const auto& r : sssrRings) {
                                                ...
                                                else { invalidBranch = true; break; }
                                            }
                                        }
                                        if (invalidBranch) break;
                                ...
                                while (!q.empty()) {
                                    int front = q.front();
                                    q.erase(q.begin());
                                    atomsToRemove.push_back(front);
                                    for (int nei : g.nodes[front].neighbors) {
                                        if (nei != exoN && !visited.count(nei) && g.nodes[nei].atomicNumber > 1) {
                                            visited.insert(nei);
                                            q.push_back(nei);
                                        }
                                    }
                            ...
                            if (invalidBranch) {
                                return {false, "", "Branched, ring, or unsaturated chains on N-substituted exocyclic amines are not supported"};
                            }

                            std::vector<QString> exoSubsts;
                            for (const auto& b : branches) {
                                ...
                                if (s == "(1-phenylmethyl)" || s == "1-phenylmethyl") {
                                    ...
                                    QString inner = s.mid(2, s.length() - 8);
                                    if (inner.startsWith("(") && inner.endsWith(")")) {
                                        inner = inner.mid(1, inner.length() - 2);
                                        s = "(" + inner + "methyl)";
                                    }
                            ...
                            for (int graphIdx : atomsToRemove) {
                                ...
                                if (indIdx != -1) {
                                    int aHandle = indigoGetAtom(bareMol, indIdx);
                                    if (aHandle > 0) handlesToRemove.push_back(aHandle);
                                    else removeOk = false;
                                } else removeOk = false;
                            }
                            ...
                            if (removeOk) {
                                ...
                                if (bareRes.success && bareRes.name.endsWith("amine")) {
                                    QString bareParent = bareRes.name;
                                    struct Substituent {
                                        QString formatted;
                                        QString sortKey;
                                    };
                                    ...
                                    int dashPos = bareParent.indexOf("-");
                                    if (dashPos > 0) {
                                        QString locantStr = bareParent.left(dashPos);
                                        bool isDigit = true;
                                        for (int i=0; i<locantStr.length(); ++i) {
                                            if (!locantStr.at(i).isDigit()) { isDigit = false; break; }
                                        }
                                        if (isDigit) {
                                            QString remainder = bareParent.mid(dashPos + 1);
                                            std::vector<QString> knownAlkyls = {"methyl", "ethyl", "propyl", "butyl", "pentyl", "hexyl", "heptyl", "octyl"};
                                            for (const QString& alkyl : knownAlkyls) {
                                                if (remainder.startsWith(alkyl)) {
                                                    substituents.push_back({locantStr + "-" + alkyl, alkyl});
                                                    bareParent = remainder.mid(alkyl.length());
                                                    break;
                                                }
                                            }
                                        }
                                    ...
                                    for (const auto& exoSub : exoSubsts) {
                                        substituents.push_back({"N-" + exoSub, exoSub});
                                    }

                                    std::vector<QString> locants;
                                    for (const auto &s : substituents) {
                                        int dIdx = s.formatted.indexOf("-");
                                        locants.push_back(dIdx >= 0 ? s.formatted.left(dIdx) : QString());
                                    }
                                    ...
                                    if (fullName.endsWith("-")) fullName.chop(1);
                                    if (!fullName.isEmpty() && bareParent.at(0).isDigit()) {
                                        fullName += "-";
                                    }
                                    fullName += bareParent;
    ...
    // --- N-substituted amides (P-66.1.1.3.1) narrow case & imide rejection ---
    {
        ...
        if (isAcyclic && countN == 1 && (countO == 1 || countO == 2) && countOtherHeavy == 0) {
            int nNode = -1;
            for (size_t i = 0; i < g.nodes.size(); ++i) {
                if (g.nodes[i].atomicNumber == 7) {
                    nNode = static_cast<int>(i);
                        ...
                        if (alkylNeighbors.size() == 2) {
                            ...
                            if (rLen == -1 || rPrimeLen1 == -1 || rPrimeLen2 == -1) {
                                if (rPrimeLen1 == -1 || rPrimeLen2 == -1) {
                                    return {false, "", "Branched or ring N-substituents on amides are not supported"};
                                } else {
                                    return {false, "", "Ring or branched acyl parents on amides are not supported"};
                                }
                            }
                            
                            if (rLen + rPrimeLen1 + rPrimeLen2 + 1 != countC) {
                                return {false, "", "Amides with additional substituents or functional groups are not supported in this phase"};
                            }
                            ...
                            QString name = prefix + chainRoot(rLen + 1) + "anamide";
                            if (rLen == 0) name = prefix + "methanamide";
                            return {true, name, ""};
                            ...
                            QString name = "N-" + chainRoot(rPrimeLen) + "yl" + chainRoot(rLen + 1) + "anamide";
                            if (rLen == 0) name = "N-" + chainRoot(rPrimeLen) + "ylmethanamide";
                            return {true, name, ""};
                        }
                    } else {
                        // countO == 2, not an imide.
        ...
        if (countS == 2 && countO == 7) {
            ...
            for (size_t i = 0; i < g.nodes.size(); ++i) {
                if (g.nodes[i].atomicNumber == 8 && g.nodes[i].neighbors.size() == 2) {
                    int nei1 = g.nodes[i].neighbors[0];
                    int nei2 = g.nodes[i].neighbors[1];
                    if (g.nodes[nei1].atomicNumber == 16 && g.nodes[nei2].atomicNumber == 16 && g.nodes[i].totalH == 0) {
                        bridgingO = static_cast<int>(i);
                        bridgingCount++;
                    }
                }
    ...
    std::map<int, int> carbonPhosphinicAcid;   // carbonNode -> phosphorusNode
    ...
    std::map<int, int> sulfinylHalideZ;        // carbonNode -> halogen atomic number

    for (size_t i = 0; i < g.nodes.size(); ++i) {
        const GraphNode &node = g.nodes[i];
        if (node.atomicNumber == 16) {
            int dblO = 0, sglO_OH = 0, sglC = 0, dblC = 0, sglS = 0, sglN = 0, sglN_unsub = 0;
            std::vector<int> cNeighbors;
            std::vector<int> halogens;
            for (size_t j = 0; j < node.neighbors.size(); ++j) {
                int nei = node.neighbors[j];
                ...
                if (nZ == 8 && order == 2) dblO++;
                    ...
                    sglN++;
                    if (order == 1 && g.nodes[nei].totalH == 2 && g.nodes[nei].neighbors.size() == 1) sglN_unsub++;
            ...
            } else if (dblO == 2 && sglN_unsub == 1 && sglC == 1 && node.neighbors.size() == 4) {
                carbonSulfonamide[cNeighbors[0]] = static_cast<int>(i);
            ...
            } else if (dblO == 1 && sglC == 1 && halogens.size() == 1 && node.neighbors.size() == 3) {
                carbonSulfinylHalide[cNeighbors[0]] = static_cast<int>(i);
            ...
            if (sglC == 1 && node.neighbors.size() == 1 && node.totalH >= 1) {
                ...
                carbonPhosphinicAcid[cNeighbors[0]] = static_cast<int>(i);
                carbonPhosphinicAcid[cNeighbors[1]] = static_cast<int>(i);
            } else {
            ...
            if (sglC == 1 && dblO == 1 && sglO_OH == 2 && node.neighbors.size() == 4) {
                ...
                carbonArsonicDihalide[cNeighbors[0]] = static_cast<int>(i);
                arsonicDihalideZ[cNeighbors[0]] = g.nodes[halogens[0]].atomicNumber;
            } else if (sglC == 2 && dblO == 1 && sglO_OH == 1 && node.neighbors.size() == 4) {
                carbonArsinicAcid[cNeighbors[0]] = static_cast<int>(i);
                carbonArsinicAcid[cNeighbors[1]] = static_cast<int>(i);
            } else {
    ...
    if (ringCount == 0 || !ringSubstituentInfos.empty()) {
        ...
        for (int i = 0; i < k; ++i) {
            ...
            if (carbonSulfonamide.count(cNode) && winningType != GroupType::SULFONAMIDE) {
                locantSubstituents[locant].append("sulfamoyl");
            }
            ...
            if (carbonSulfinylHalide.count(cNode) && winningType != GroupType::SULFINYL_HALIDE) {
            ...
            for (size_t j = 0; j < g.nodes[cNode].neighbors.size(); ++j) {
                ...
                if (carbonSulfonamide.count(cNode) && carbonSulfonamide[cNode] == nei) continue;
        ...
        if (winningType != GroupType::NONE) {
            ...
            if (winningType == GroupType::ACYL_HALIDE) {
                ...
                acylHalideZForSfx = sulfinylHalideZ[bestSig.parentChain[0]];
                for (int pc : bestSig.parentChain) {
                    if (sulfinylHalideZ.count(pc)) { acylHalideZForSfx = sulfinylHalideZ[pc]; break; }
                }
            } else if (winningType == GroupType::PHOSPHONIC_DIHALIDE) {
                ...
                acylHalideZForSfx = arsonicDihalideZ[bestSig.parentChain[0]];
                for (int pc : bestSig.parentChain) {
                    if (arsonicDihalideZ.count(pc)) { acylHalideZForSfx = arsonicDihalideZ[pc]; break; }
                }
            }
        ...
        } else if (winningType == GroupType::PHOSPHINIC_ACID) {
            int pNode = -1;
            for (auto it = carbonPhosphinicAcid.begin(); it != carbonPhosphinicAcid.end(); ++it) {
                pNode = it->second;
                break;
            }
            if (pNode != -1) {
                ...
                for (int nei : g.nodes[pNode].neighbors) {
                    if (g.nodes[nei].atomicNumber == 6) {
                        pCarbons.push_back(nei);
                    }
                }
                if (pCarbons.size() == 2) {
                    QString name1 = nameBranchGraph(g, pCarbons[0], pNode, allSSSRRings);
                    QString name2 = nameBranchGraph(g, pCarbons[1], pNode, allSSSRRings);
                    if (name1.isEmpty() || name2.isEmpty()) {
                        return {false, "", "Unsupported phosphinic acid alkyl group."};
                    }
                    ...
                    if (clean1 == clean2) {
                        fullName = stereoRes.prefix + "di" + clean1 + "phosphinic acid";
                    } else {
                        QString a = alphabetizationKey(clean1).toLower() < alphabetizationKey(clean2).toLower() ? clean1 : clean2;
                        QString b = alphabetizationKey(clean1).toLower() < alphabetizationKey(clean2).toLower() ? clean2 : clean1;
                        fullName = stereoRes.prefix + a + "(" + b + ")phosphinic acid";
                    }
                    return {true, fullName, ""};
                }
            }
            return {false, "", "Unsupported phosphinic acid geometry."};
        } else if (winningType == GroupType::ARSINIC_ACID) {
            int asNode = -1;
            for (auto it = carbonArsinicAcid.begin(); it != carbonArsinicAcid.end(); ++it) {
                asNode = it->second;
                break;
            }
            if (asNode != -1) {
                ...
                for (int nei : g.nodes[asNode].neighbors) {
                    if (g.nodes[nei].atomicNumber == 6) {
                        asCarbons.push_back(nei);
                    }
                }
                if (asCarbons.size() == 2) {
                    QString name1 = nameBranchGraph(g, asCarbons[0], asNode, allSSSRRings);
                    QString name2 = nameBranchGraph(g, asCarbons[1], asNode, allSSSRRings);
                    if (name1.isEmpty() || name2.isEmpty()) {
                        return {false, "", "Unsupported arsinic acid alkyl group."};
                    }
                    ...
                    if (clean1 == clean2) {
                        fullName = stereoRes.prefix + "di" + clean1 + "arsinic acid";
                    } else {
                        QString a = alphabetizationKey(clean1).toLower() < alphabetizationKey(clean2).toLower() ? clean1 : clean2;
                        QString b = alphabetizationKey(clean1).toLower() < alphabetizationKey(clean2).toLower() ? clean2 : clean1;
                        fullName = stereoRes.prefix + a + "(" + b + ")arsinic acid";
                    }
                    return {true, fullName, ""};
                }
            }
    ...
    if ((ringCount == 3 && allSSSRNodes.size() == 10) || (ringCount == 5 && allSSSRNodes.size() == 8)) {
        ...
        if (validSkeleton) {
            auto myGroupRank = [](GroupType gt) -> int {
                ...
                if (gt == GroupType::THIOAMIDE) return 3;
                if (gt == GroupType::HYDRAZIDE) return 4;
                if (gt == GroupType::KETONE) return 5;
                if (gt == GroupType::ALCOHOL) return 6;
                if (gt == GroupType::AMINE) return 7;
                return 99;
            };
            ...
            for (int rIdx : allSSSRNodes) {
                bool hasDblO = false, hasSglO_OH = false, hasSglN = false, hasDblS = false;
                ...
                else if (hasDblS && hasSglN) myCarbonGroup[rIdx] = GroupType::THIOAMIDE;
            ...
            for (int rIdx : allSSSRNodes) {
                for (size_t j = 0; j < g.nodes[rIdx].neighbors.size(); ++j) {
                    ...
                    if (principalCarbons.count(rIdx)) {
                        ...
                        if (winningType == GroupType::THIOAMIDE && (z == 16 || z == 7)) isPrincipalAtom = true;
