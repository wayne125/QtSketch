...
- **P-44 ring-vs-chain seniority — partially covered as of Phase 56**: the P-44.1.1 instance-count decision and the P-44.1.2.2 ring-wins-on-genuine-tie default are implemented, and the chain-wins case is now named for `ACID`, `AMIDE`, `NITRILE`, `ALDEHYDE`, `KETONE`, `ALCOHOL`, `THIOL`, `AMINE`, `THIAL`, `THIONE`, `SULFONIC_ACID`, `ESTER`, and `ACYL_HALIDE` (generalized in Phase 54/56/58 from acid-only via a shared `nameAcyclicChainParentWithSubstituents`). Still gaps: `BORONIC_ACID` and `PHOSPHINE` (named via entirely separate early-return logic) remain unsupported and reject cleanly; `P-44.1.2` heteroatom-skeleton seniority (e.g. Si chain vs C ring) is not implemented; chain-wins still requires a single ring<->chain attachment and a 5/6-membered monocyclic ring (a polysubstituted naphthalene + acid chain still rejects; fused/bridged/polycyclic-ring-as-substituent is a separate future phase).
