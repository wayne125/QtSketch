...
| P-14.7 | Indicated and added indicated hydrogen | ◐ | `preferIndicatedHydrogenLocant` + Phase 35/38's `nH-` prefix citation for NH-bearing fused systems. Same real rule as FR-9 below — Blue Book P-14.7 is the general statement, FR-9 is fusion-nomenclature's own restatement of it. General (non-fusion) indicated hydrogen, and "added" indicated hydrogen for suffix-bearing rings, not covered. |
...
| P-22.2 | Heteromonocyclic parent hydrides (Hantzsch-Widman) | ◐ | `classifyMonocyclicHeteroRing` recognizes a curated set of retained/systematic names (furan, thiophene, selenophene, tellurophene [Phase 47], pyrrole, pyridine, pyrazole, imidazole, pyridazine, pyrimidine, pyrazine, oxazole, isoxazole, thiazole, isothiazole, selenazole, isoselenazole [Phase 47] + saturated piperidine/pyrrolidine/THF/THT). **Gap**: no general Hantzsch-Widman stem construction (`tryGeneralHeterocycle` only emits a `<GENERAL_HETEROCYCLE>` placeholder for other heteroatom/ring-size combinations outside this curated list — larger rings, P/As/Sb/Bi/Si/Ge/Sn/Pb/B heteroatoms, uncommon heteroatom counts/positions). |
| P-23.0-P-23.2 | Definitions, terminology, naming/numbering von Baeyer hydrocarbons | ◐ | Phase 27: `bicyclo[a.b.c]alkane` — saturated, unsubstituted, all-carbon, exactly 2 bridgeheads only. |
...
| P-24.0-P-24.2 | Introduction, definitions, spiro with only monocyclic components | ◐ | Phase 28: `spiro[a.b]alkane` — saturated, unsubstituted, single two-ring spiro only. |
...
| Section | Title | Status | Note |
|---|---|---|---|
| P-41 | Seniority order for classes | ◐ | `GroupType` enum + `groupRank`: sulfonic > carboxylic > boronic > ester > acyl halide > amide > nitrile > aldehyde > thial > ketone > thione > alcohol > thiol > amine > phosphine. Not the full official class list (e.g. no radicals/anions/cations classes, since those are rejected earlier anyway). |
| P-42 | Seniority order for acids | ◐ | Partial — sulfonic > carboxylic > boronic ordering exists, but not the full acid-subclass seniority table (e.g. chalcogen/replacement-analogue acids). |
...
| **P-44** | **Seniority order for parent structures (ring vs. chain, etc.)** | **✗** | This is the exact real number for a gap already known and explicitly rejected in code: `"A chain-based principal group outranks the ring in this structure; ring-vs-chain seniority is not yet supported in Phase 2."` Ring-vs-chain, and all the other parent-structure-seniority tie-breaks P-44 covers, are unimplemented. |
...
P-53 Selecting preferred retained names of parent hydrides, P-54 Selecting the
...
| P-67.1 | Mononuclear noncarbon oxoacids | ◐ | Sulfonic acid (`GroupType::SULFONIC_ACID`) and boronic acid (`GroupType::BORONIC_ACID`) covered; the general noncarbon-oxoacid system (phosphonic, phosphinic, arsonic, etc.) is not — phosphine (`GroupType::PHOSPHINE`) covers the amine-analogue class, not the oxoacid class. |
...
| FR-9 | Indicated hydrogen | ◐ | ≈ Blue Book P-14.7 and P-25.7. `preferIndicatedHydrogenLocant` branch in `computePeripheralNumbering` + Phase 35's `nH-` prefix for NH-bearing fused systems (pyrrole/imidazole/pyrazole). General indicated-hydrogen (mancude-ring adducts beyond this) not covered. |
...
- **P-44 (seniority order for parent structures) unsupported** — chain principal group outranking a ring aborts naming. This is the real Blue Book number for a gap previously only described informally.
...
2. **P-25.4/P-25.5 (peri-fused / bridged-fused / 3-component-limit systems)** — needs
   ...
   without them).
3. **P-22.2 general Hantzsch-Widman stem construction** — replace `tryGeneralHeterocycle`'s placeholder with real stem+prefix construction for heteroatoms/ring sizes outside the curated list.
4. **P-23/P-24 von Baeyer & spiro**: add unsaturation, heteroatoms, and substituents (currently saturated+unsubstituted+all-carbon only for both — P-23.3-P-23.7 and P-24.3-P-24.8 entirely unimplemented).
5. **Blue Book section P-44 (ring-vs-chain and other parent-structure seniority)** — now that this has a real section number, worth scoping directly against its actual text rather than leaving it as an ad hoc rejection message. (Not to be confused with this codebase's own Phase 44, above.)
